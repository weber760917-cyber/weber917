...
const PREVIEW_FILE = join(__dirname, '../data/wc-preview.json');

function getTomorrowDate() {
  // 台灣時間明天，轉為 ESPN API 格式 YYYYMMDD
  const twOffset = 8 * 60 * 60 * 1000;
  const twTomorrow = new Date(Date.now() + twOffset);
...
async function main() {
  ...
  for (const match of matches) {
    ...
    try {
      ...
      results.push({ ...match, analysis: '本場分析暫時無法取得。' });
    }
  ...
  writeFileSync(PREVIEW_FILE, JSON.stringify(output, null, 2), 'utf8');
